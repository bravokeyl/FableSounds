...
function bk_assign_voucher_to_user($username,$ac_id,$product_id,$product_sku){
  ...
  if(is_array($sku_arr) && 0 < count($sku_arr)){
    foreach($sku_arr as $key => $pid){
      $slug = strtolower($username).'-voucher-code-id-'.$ac_id.'-'.$pid.'-'.wp_rand(100,99999);
    	$title = strtoupper($username)."-".$ac_id."-".$product_id."-".$product_sku."-for-".$pid;
      if( null == get_page_by_title( $title ) ) {
        if(!empty($pid)){
          $bk_wclogger->add('fablesounds','Debug: Creating Voucher - Eligible Product key '.$key.' - value pid: - '.$pid);
...
function bk_check_add_to_cart($cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
  ...
  if( ($codes_available && (0 < count($codes_available) )) || $is_product_update ){
